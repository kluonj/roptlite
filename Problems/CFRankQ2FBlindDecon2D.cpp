...
namespace ROPTLITE {
	...
	realdp CFRankQ2FBlindDecon2D::f(const Variable &x) const
	{
        ...
        Vector FBU = ((*Bptr) * U).Reshape(n1, n2).GetFFT2D(FFTW_FORWARD).Reshape(L), FCV = ((*Cptr) * V).Reshape(n1, n2).GetInvHaarFWT().GetFFT2D(FFTW_BACKWARD).Reshape(L);
	...
	Vector &CFRankQ2FBlindDecon2D::EucGrad(const Variable &x, Vector *result) const
	{
        ...
        Vector EGFTU = ntwo * (ymdiagBXC.GetConj().GetDiagTimesM(FBU, GLOBAL::L).Reshape(n1, n2).GetFFT2D(FFTW_FORWARD).GetHaarFWT().Reshape(L).GetTranspose() * (*Cptr)).GetTranspose();
	...
	{ /*The Hessian of the penalty term is not considered here since the penalty is only C^1, not C^2 */
        ...
        Vector ymdiagBXC = x.Field("ymdiagBXC"), FBU = x.Field("FBU"), FCV = x.Field("FCV");
        Vector FBdU = ((*Bptr) * dU).Reshape(n1, n2).GetFFT2D(FFTW_FORWARD).Reshape(L), FCdV = ((*Cptr) * dV).Reshape(n1, n2).GetInvHaarFWT().GetFFT2D(FFTW_BACKWARD).Reshape(L);
        ...
        Vector dJ1TU = two * (dDBUVC.GetConj().GetDiagTimesM(FBU, GLOBAL::L).Reshape(n1, n2).GetFFT2D(FFTW_FORWARD).GetHaarFWT().Reshape(L).GetTranspose() * (*Cptr)).GetTranspose();
        Vector J1TdU = ntwo * (ymdiagBXC.GetConj().GetDiagTimesM(FBdU, GLOBAL::L).Reshape(n1, n2).GetFFT2D(FFTW_FORWARD).GetHaarFWT().Reshape(L).GetTranspose() * (*Cptr)).GetTranspose();
