...
//        IsVectTranSmooth = true;
    ...
    Vector &CSymFixedRankQ::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        printf("warning:CSymFixedRankQ::VecTranDiffRetAdjoint has not been done!\n");
    ...
    LinearOPE &CSymFixedRankQ::TranHInvTran(const Variable &x, const Vector &etax, const Variable &y, const LinearOPE &Hx, LinearOPE *result) const
    {
        ...
        if (HasHHR)
        {
            printf("Warning: CSymFixedRankQ::TranHInvTran: Transport a linear operator using vector transport with locking condition has not been done!\n");
            return Manifold::TranHInvTran(x, etax, y, Hx, result);
        }
...
//            return *result;
    ...
    Vector &CSymFixedRankQ::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
