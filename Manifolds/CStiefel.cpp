...
//        IsVectTranSmooth = true;
	...
	Vector &CStiefel::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
		if (retraction == CSTIE_QF)
			return qfVecTranDiffRetAdjoint(x, etax, y, xiy, result);

		if (retraction == CSTIE_POLAR)
			return PolarVecTranDiffRetAdjoint(x, etax, y, xiy, result);

		printf("Error: VecTranDiffRetAdjoint has not been done!\n");
        return Projection(x, xiy, result);
	};

	Vector &CStiefel::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
		if (retraction == CSTIE_QF)
		...
		printf("Error: VecTranDiffRet has not been done!\n");
        return Manifold::VecTranDiffRet(x, etax, y, xix, result, IsEtaXiSameDir);
	};
    ...
    Vector &CStiefel::ObtainNorVerIntr(const Variable &x, const Vector &etax, Vector *result) const
    {
        ...
        {
            resultptr[idx] = xTetaxptr[i + i * p].real();
            idx++;
        }
        for(integer i = 0; i < p; i++)
        {
            ...
            {
                resultptr[idx] = (xTetaxptr[i + j * p].real() + xTetaxptr[j + i * p].real()) / r2;
                idx++;
            }
        }
        for(integer i = 0; i < p; i++)
        {
            for(integer j = i + 1; j < p; j++)
            {
                resultptr[idx] = (xTetaxptr[i + j * p].imag() - xTetaxptr[j + i * p].imag()) / r2;
                idx++;
            }
        }
        return *result;
    ...
    Vector &CStiefel::ObtainNorVerExtr(const Variable &x, const Vector &intretax, Vector *result) const
    {
        ...
        for(integer i = 0; i < p; i++)
        {
            xTetaxptr[i + i * p] = realdpcomplex{intretaxptr[idx],             xTetaxptr[i + i * p].imag()};
            idx++;
        }
        for(integer i = 0; i < p; i++)
        {
            for(integer j = i + 1; j < p; j++)
            {
                xTetaxptr[i + j * p] = realdpcomplex{intretaxptr[idx] / r2,                 xTetaxptr[i + j * p].imag()};
                xTetaxptr[j + i * p] = realdpcomplex{xTetaxptr[i + j * p].real(),                 xTetaxptr[j + i * p].imag()};
                idx++;
            }
        }
        for(integer i = 0; i < p; i++)
        {
            for(integer j = i + 1; j < p; j++)
            {
                xTetaxptr[i + j * p] = realdpcomplex{                xTetaxptr[i + j * p].real(), intretaxptr[idx] / r2};
                xTetaxptr[j + i * p] = realdpcomplex{                xTetaxptr[j + i * p].real(), - xTetaxptr[i + j * p].imag()};
                idx++;
            }
        }
    ...
    Vector &CStiefel::qfVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
    ...
    Vector &CStiefel::DiffqfRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        ...
        for (integer i = 0; i < p; i++)
        {
            YtVRinvptr[i + p * i] = realdpcomplex{-YtVRinvptr[i + p * i].real(), 0};
            for (integer j = i + 1; j < p; j++)
            {
                YtVRinvptr[i + p * j] = realdpcomplex{-YtVRinvptr[j + p * i].real() - YtVRinvptr[i + p * j].real(),                 YtVRinvptr[i + p * j].imag()};
                YtVRinvptr[j + p * i] = realdpcomplex{0,                 YtVRinvptr[j + p * i].imag()};
                YtVRinvptr[i + p * j] = realdpcomplex{                YtVRinvptr[i + p * j].real(), YtVRinvptr[j + p * i].imag() - YtVRinvptr[i + p * j].imag()};
                YtVRinvptr[j + p * i] = realdpcomplex{                YtVRinvptr[j + p * i].real(), 0};
            }
        }
	...
	Vector &CStiefel::PolarVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
	...
	Vector &CStiefel::DiffPolarRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
        ...
        return Manifold::VecTranDiffRet(x, etax, y, xix, result);
	};
	...
	Vector &CStiefel::ObtainIntrHHR(const Variable &x, const Vector &etax, Vector *result) const
	{
        ...
        for (integer i = 0; i < p; i++)
        {
            resultptr[idx] = tmpptr[i + i * n].imag();
            idx++;
        }
		...
		for (integer i = 0; i < p; i++)
		{
			for (integer j = i + 1; j < p; j++)
			{
                resultptr[idx] = r2 * (tmpptr[j + i * n].real() - tmpptr[i + j * n].real()) / 2;
                idx++;
                resultptr[idx] = r2 * (tmpptr[j + i * n].imag() + tmpptr[i + j * n].imag()) / 2;
                idx++;
			}
		}
		...
		for (integer i = 0; i < p; i++)
		{
			...
			{
				resultptr[idx] = tmpptr[j + i * n].real();
				idx++;
                resultptr[idx] = tmpptr[j + i * n].imag();
                idx++;
			}
		}
        return *result;
	...
	Vector &CStiefel::ObtainExtrHHR(const Variable &x, const Vector &intretax, Vector *result) const
	{
        ...
        for (integer i = 0; i < p; i++)
        {
            resultptr[i + i * n] = realdpcomplex{            resultptr[i + i * n].real(), intretaxptr[idx]};
            resultptr[i + i * n] = realdpcomplex{0,             resultptr[i + i * n].imag()};
            idx++;
        }
        ...
        for (integer i = 0; i < p; i++)
        {
            for (integer j = i + 1; j < p; j++)
            {
                resultptr[j + i * n] = realdpcomplex{intretaxptr[idx] / r2,                 resultptr[j + i * n].imag()};
                resultptr[i + j * n] = realdpcomplex{- resultptr[j + i * n].real(),                 resultptr[i + j * n].imag()};
                idx++;
                resultptr[j + i * n] = realdpcomplex{                resultptr[j + i * n].real(), intretaxptr[idx] / r2};
                resultptr[i + j * n] = realdpcomplex{                resultptr[i + j * n].real(), resultptr[j + i * n].imag()};
                idx++;
            }
        }
        ...
        for (integer i = 0; i < p; i++)
        {
            for (integer j = p; j < n; j++)
            {
                resultptr[j + i * n] = realdpcomplex{intretaxptr[idx],                 resultptr[j + i * n].imag()};
                idx++;
                resultptr[j + i * n] = realdpcomplex{                resultptr[j + i * n].real(), intretaxptr[idx]};
                idx++;
            }
        }
